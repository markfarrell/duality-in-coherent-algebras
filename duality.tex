...
\frametitle{Strongly-Regular Graphs}
	...
	\only<1>{
		...
		Assume that there is a duality mapping on the adjacency algebra $\mathcal{A}$ of $G$.
	...
	\only<2>{
		...
		is the dual character table of $\mathcal{A}$ relative to $P$.
	...
	\only<3>{
		...
		\begin{proposition}
		Suppose that there is a duality mapping on the adjacency algebra $\mathcal{A}$ of a strongly-regular graph $G$.
		Then $\mathcal{A}$ is self-dual if and only if the valency of $G$ is the valency of $\overline{G}$.
